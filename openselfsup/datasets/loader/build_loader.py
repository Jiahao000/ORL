    ...
    def __iter__(self):
        ...
        for next_input_dict in self.loader:
            with torch.cuda.stream(stream):
                for k in next_input_dict.keys():
                    if k.endswith('label') or k == 'idx':
                        continue
                    data = next_input_dict[k].cuda(non_blocking=True)
                    next_input_dict[k] = data.float().sub_(self.mean).div_(self.std)
